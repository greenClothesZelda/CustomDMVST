...
class MyGraphDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for idx, time in enumerate(json_data['meta']['hours']):
            demand_t =torch.tensor(json_data['x'][idx], dtype=torch.float).div(max_demand)
            one_hot_t = torch.eye(total_nodes)
            combined_features = torch.cat([demand_t.unsqueeze(1), one_hot_t], dim=1)
            all_demands.append(combined_features)  # 정규화
        ...
        for i in range(len(all_demands) - self.time_step):
            x = torch.stack(all_demands[i : i + self.time_step]) #[time_step, num_nodes, num_nodes + 1]
        ...
        torch.save({'max_demand': max_demand, 'count_data': count_data, 'coverage': json_data['meta']['coverage']}, self.processed_paths[1])
